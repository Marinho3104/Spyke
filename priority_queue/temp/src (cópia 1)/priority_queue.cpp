...
priority_queue::Priority_Queue::Priority_Queue( const uint8_t& slots_count, const uint32_t& max_items_per_slot ) : slots_count( slots_count ) {
...
void priority_queue::Priority_Queue::add_item( std::unique_ptr< Item >&& item, const uint8_t& priority ) {

  ENSURE( ( priority < slots_count ) );

  slots_mut.get()[ priority ].add_item( std::move( item ) );

}

std::unique_ptr< priority_queue::Item > priority_queue::Priority_Queue::pop() {}
